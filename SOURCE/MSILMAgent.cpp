...
Rule
MSILMAgent::cognittion_say(std::vector<Rule>& internals) {
	try {
...
Rule
MSILMAgent::say_think_meaning(std::vector<Rule>& internals) {
	if (LOGGING_FLAG) {
...
void
MSILMAgent::cognittion_hear(std::vector<Rule>& utterances, std::vector<std::vector<Rule> >& meaning_lists, std::vector<Rule>& all_meanings) {
...
void
MSILMAgent::symmetry_bias_think(std::vector<Rule>& utterances, std::vector<std::vector<Rule> >& meaning_lists, std::vector<Rule>& reference, std::vector<std::vector<Rule> >& return_rules) {
	if (LOGGING_FLAG) {
...
void
MSILMAgent::ucsymmetry_bias_inference(std::vector<Rule>& utterances, std::vector<std::vector<Rule> >& meaning_lists, std::vector<Rule>& reference, std::vector<std::vector<Rule> >& term_pairs, std::vector<std::vector<std::vector<Rule> > >& meaning_pair_lists, std::vector<std::vector<double> >& meaning_distance_lists) {
	if (LOGGING_FLAG) {
		LogBox::push_log("USING SYMMETRY BIAS THINK");
		LogBox::push_log("PROSPECTIVE MEANINGS:\n" + tr_vector_Rule_to_string(meaning_lists));
	}
