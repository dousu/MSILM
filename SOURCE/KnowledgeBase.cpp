...
KnowledgeBase::chunking(Rule &src, Rule &dst)
{
    ...
    case TYPE2:
    {
        ...
        noun.set_noun(Variable(base.internal[idiff_index].get<Variable>()).get_cat_id(), targ.internal[idiff_index],
                      noun_ex);
                               ...
                               std::map<int, bool> &unified_cat)
{
    ...
    while (it != DB.end())
    {
        ...
        {
            if (temp.internal[i].type() == ELEM_TYPE::VAR_TYPE && unified_cat.find(Variable(temp.internal[i].get<Variable>()).get_cat_id()) != unified_cat.end())
            {
                if (LOGGING_FLAG)
                {
                    LogBox::push_log("MERGE-> " + (*it).to_s());
        ...
        if (modified)
        {
            ...
            {
                if ( //find unified cat
                    temp.external[j].type() == ELEM_TYPE::CAT_TYPE && unified_cat.find(Nonterminal(temp.external[j].get<Nonterminal>()).get_cat_id()) != unified_cat.end())
                {
                    temp.external[j] = Nonterminal(base_word.cat, Nonterminal(temp.external[j].get<Nonterminal>()).get_obj_id());
                }
            }
            if (LOGGING_FLAG)
            {
    ...
    while (it != wordDB.end())
    {
        if (word_dic.find((*it).cat) != word_dic.end())
        {
            word_dic[(*it).cat].insert(
                ItemType::value_type(Mean((*it).internal.front().get<Mean>()).get_obj_id(), *it));
        }
        else
        {
            ItemType temp;
            temp.insert(ItemType::value_type(Mean((*it).internal.front().get<Mean>()).get_obj_id(), *it));
...
void KnowledgeBase::ground_with_pattern(Rule &src, PatternType &pattern)
{
    ...
    while (sent_ex_it != base_rule.external.end())
    {
        ...
        else if ((*sent_ex_it).type() == ELEM_TYPE::CAT_TYPE)
        {
            ...
            {
                if (Variable((*pattern_it).internal.front().get<Variable>()) == Nonterminal((*sent_ex_it).get<Nonterminal>()))
                {
...
KnowledgeBase::construct_grounding_patterns(Rule &src)
{
    ...
    //srcチェック
    std::for_each(std::begin(src.internal), std::end(src.internal), [](Element &el) {
        if (el.type() == ELEM_TYPE::VAR_TYPE)
        {
            std::cerr << "src includes some variables in KnowledgeBase::construct_grounding_patterns(Rule&)" << std::endl;
            exit(1);
        }
    });
    ...
    while (sent_it != sentenceDB.end())
    {
        ...
        {
            if ((*sent_it).internal[index].type() != ELEM_TYPE::VAR_TYPE && src.internal[index] != (*sent_it).internal[index])
            {
                filted = false;
            }
        }
